...
from .models import AOE, Candidacy, Election, election_window_instants
...
class ElectionAdminForm(forms.ModelForm):
    ...
    dates (see `elections.models.aoe_instant`) — the admin's default
    split date/time widget would make a staff member work out that
    conversion by hand, exactly the busywork `create_election` exists to
    avoid. This form exposes the same four dates instead and computes the
    instants itself, so the admin and the management command behave the
    same way.
    """
    ...
    # AdminSplitDateTime — there's no time to enter, only a date, AOE.
    nomination_opens = forms.DateField(
        widget=AdminDateWidget, help_text="AOE. The window opens once this date has begun everywhere."
    )
    nomination_closes = forms.DateField(
        widget=AdminDateWidget, help_text="AOE. The window stays open through all of this date."
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            self.fields["nomination_opens"].initial = self.instance.nomination_opens_at.astimezone(AOE).date()
            self.fields["nomination_closes"].initial = (
                self.instance.nomination_closes_at.astimezone(AOE).date() - ONE_DAY
            )
            self.fields["voting_opens"].initial = self.instance.voting_opens_at.astimezone(AOE).date()
